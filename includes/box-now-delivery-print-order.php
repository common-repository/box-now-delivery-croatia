...
function boxnow_print_voucher_pdf()
{
  ...
  if (empty($parcel_id)) {
    echo esc_html__('Parcel ID was not found!', 'box-now-delivery-croatia');
    exit;
  }
  ...
  if (is_wp_error($auth_response) || 200 != wp_remote_retrieve_response_code($auth_response)) {
    echo esc_html__('Error: Authentication failed.', 'box-now-delivery-croatia');
    exit;
  }
  ...
  if (is_wp_error($response)) {
    echo esc_html__('Error: %s.'  , 'box-now-delivery-croatia');
